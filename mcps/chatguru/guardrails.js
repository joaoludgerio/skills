// ─── GUARDRAILS DE ENVIO ─────────────────────────────────────────────────────
// Regras de seguranca para envio de mensagens via ChatGuru MCP (Playwright).
// Baseado em mcps/whatsapp/src/guardrails.js — mesmas constantes e logica.

import fs from "fs";
import os from "os";
import path from "path";

// ─── CONSTANTES FIXAS (nao configuraveis) ────────────────────────────────────

const RATE_LIMIT_PER_MINUTE = 10;       // maximo de envios por minuto
const MAX_RECIPIENTS_PER_DAY = 50;      // maximo de destinatarios unicos por dia
...
const STATE_FILE = path.join(os.tmpdir(), "chatguru-mcp-state.json");
const AUDIT_FILE = path.join(os.homedir(), ".chatguru-mcp-audit.jsonl");
...
export function checkRateLimit() {
  ...
  if (sendTimestamps.length >= RATE_LIMIT_PER_MINUTE) {
    throw new Error(
      `Rate limit atingido: maximo ${RATE_LIMIT_PER_MINUTE} mensagens por minuto. ` +
...
// ─── LIMITE DIARIO DE DESTINATARIOS ────────────────────────────────────────
...
export function checkDailyRecipientLimit(chatId) {
  ...
  if (!state.recipients.includes(chatId)) {
    if (state.recipients.length >= MAX_RECIPIENTS_PER_DAY) {
      throw new Error(
        `Limite diario atingido: maximo ${MAX_RECIPIENTS_PER_DAY} destinatarios unicos por dia. ` +
        `Ja enviado para ${state.recipients.length} chats hoje.`
      );
    }
...
export function checkMessageLength(text, confirmed) {
  if (text.length > MSG_LIMIT_HARD) {
    throw new Error(
      `Mensagem muito longa (${text.length} caracteres). Maximo permitido: ${MSG_LIMIT_HARD}. ` +
      `Divida em partes menores.`
    );
  }
  if (text.length > MSG_LIMIT_SOFT && !confirmed) {
    throw new Error(
      `Mensagem longa (${text.length} caracteres, limite padrao: ${MSG_LIMIT_SOFT}). ` +
      `Use confirmed: true para confirmar o envio de mensagem longa (max ${MSG_LIMIT_HARD} chars).`
    );
  }
}

// ─── ANTI-LOOP ─────────────────────────────────────────────────────────────
// Bloqueia o mesmo texto para o mesmo chat em menos de 60s.
...
export function checkAntiLoop(chatId, text) {
  ...
  if (duplicate) {
    ...
    throw new Error(
      `Anti-loop: esta mensagem ja foi enviada para este chat ha ${secsAgo}s. ` +
...
// ─── BLOQUEIO DE CONTEUDO SENSIVEL ─────────────────────────────────────────

const SENSITIVE_PATTERNS = [
  { pattern: /\b\d{4}[\s\-]?\d{4}[\s\-]?\d{4}[\s\-]?\d{4}\b/, label: "numero de cartao de credito" },
...
export function checkSensitiveContent(text, confirmed) {
  if (confirmed) return; // usuario confirmou explicitamente
  const found = [];
  ...
  if (found.length > 0) {
    throw new Error(
      `Conteudo potencialmente sensivel detectado: ${found.join(", ")}.\n` +
