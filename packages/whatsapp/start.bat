...
echo [whatsapp-mcp] pm2 nao encontrado. Instalando...
...
if not defined PM2_BIN (
    echo [whatsapp-mcp] ERRO: pm2 nao encontrado mesmo apos instalacao. Verifique npm.
    exit /b 1
)
...
if %ERRORLEVEL% NEQ 0 (
    echo [whatsapp-mcp] Iniciando daemon WebSocket...
    "%PM2_BIN%" start "%DIR%src\ws-daemon.js" --name whatsapp-ws-daemon
    "%PM2_BIN%" save
) else (
...
set /a RETRIES+=1
if %RETRIES% GEQ 10 (
    echo [whatsapp-mcp] Daemon nao respondeu apos 10s. Continuando mesmo assim...
    goto DAEMON_READY
)
